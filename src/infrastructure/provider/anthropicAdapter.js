...
export function createAnthropicAdapter(){
  return {
    ...
    async sendChat(req){
      ...
      const msgPayload = normalized.map(m=> ({ role: m.role, content: [{ type:'text', text: m.content }] }))
      ...
      try {
        const R = budget && typeof budget.remainingContext === 'number' ? budget.remainingContext : undefined
        const otpm = req.meta && typeof req.meta.otpm === 'number' ? req.meta.otpm : undefined
        const utmo = userOverrides && typeof userOverrides.maxOutputTokens === 'number' ? userOverrides.maxOutputTokens : undefined
        ...
        if(!capInt || capInt < 1) throw new ProviderError('context overflow (no output room)','bad-request')
      ...
      try {
        if(typeof window !== 'undefined'){
          window.__maichatLastRequest = { at: Date.now(), model, json: payloadStr, placeholderInsertions: insertedPlaceholders }
        }
      } catch{}
      let resp
      try {
