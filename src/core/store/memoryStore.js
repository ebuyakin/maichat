...
export class MemoryStore { constructor(){ this.pairs=new Map(); this.topics=new Map(); this.children=new Map(); this.emitter=new Emitter(); this.rootTopicId='root'; const root=createTopic({ id:this.rootTopicId, name:'Root', parentId:null }); root.directCount=0; root.totalCount=0; this.topics.set(this.rootTopicId, root); if(!this.children.has(null)) this.children.set(null,new Set()); this.children.get(null).add(this.rootTopicId); this.emitter.emit('topic:add', root) }
  ...
  rebuildLastActiveAt(){
    for(const t of this.topics.values()){ t.lastActiveAt = 0 }
    // For each pair, set topic's lastActiveAt to max of direct pairs
    for(const p of this.pairs.values()){ const t=this.topics.get(p.topicId); if(t) t.lastActiveAt = Math.max(t.lastActiveAt||0, p.createdAt||0) }
    ...
    while(changed && guard++<20){
      ...
      for(const t of this.topics.values()){
        ...
        if(kids){ for(const cid of kids){ const ct=this.topics.get(cid); if(ct) maxVal=Math.max(maxVal, ct.lastActiveAt||0) } }
        if(maxVal!==(t.lastActiveAt||0)){ t.lastActiveAt=maxVal; changed=true }
      }
    }
    this.emitter.emit('topic:lastActive')
  }
  _bumpLastActiveForTopic(topicId, ts){ if(!Number.isFinite(ts)) return; let cur=this.topics.get(topicId); if(!cur) return; if((cur.lastActiveAt||0) < ts){ cur.lastActiveAt = ts } while(cur.parentId){ cur=this.topics.get(cur.parentId); if(!cur) break; if((cur.lastActiveAt||0) < ts) cur.lastActiveAt = ts; else break }
  }
  _recalcLastActiveForTopicSubtree(topicId){ // when decreasing activity (delete or move away)
    // Recompute exact subtree max, then propagate upward exact max of children
    const t=this.topics.get(topicId); if(!t) return;
    const subtreeMax=(id)=>{
      const node=this.topics.get(id); if(!node) return 0; let maxv=0; // own direct pairs
      for(const p of this.pairs.values()){ if(p.topicId===id) maxv=Math.max(maxv, p.createdAt||0) }
      const kids=this.children.get(id); if(kids){ for(const cid of kids){ maxv=Math.max(maxv, subtreeMax(cid)) } }
      return maxv
    }
    const newVal=subtreeMax(topicId); const node=this.topics.get(topicId); if(node) node.lastActiveAt=newVal;
    ...
    while(cur){ let maxv=0; const kids=this.children.get(cur.id); if(kids){ for(const cid of kids){ const ct=this.topics.get(cid); if(ct) maxv=Math.max(maxv, ct.lastActiveAt||0) } }
      for(const p of this.pairs.values()){ if(p.topicId===cur.id) maxv=Math.max(maxv, p.createdAt||0) }
      cur.lastActiveAt=maxv; cur=cur.parentId ? this.topics.get(cur.parentId) : null }
    this.emitter.emit('topic:lastActive')
  }
