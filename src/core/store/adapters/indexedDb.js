...
export class IndexedDbAdapter {
  ...
  async getAllTopics(){ await this.init(); return new Promise((res,rej)=>{ const r = this._tx(TOPICS,'readonly').getAll(); r.onsuccess=()=>res(r.result); r.onerror=()=>rej(r.error) }) }
  async saveMeta(record){ await this.init(); await new Promise((res,rej)=>{ const r = this._tx(META,'readwrite').put(record); r.onsuccess=()=>res(); r.onerror=()=>rej(r.error) }) }
  async getMeta(name){ await this.init(); return new Promise((res,rej)=>{ const r = this._tx(META,'readonly').get(name); r.onsuccess=()=>res(r.result); r.onerror=()=>rej(r.error) }) }
}
