...
function normalize(state){
  ...
  if(!state.activeModel || !state.models[state.activeModel]?.enabled){ const first = BASE_MODELS.find(m=> state.models[m.id]?.enabled); if(first) state.activeModel = first.id }
  return state
}
...
export function listModels(){ return BASE_MODELS.map(m=> ({ id:m.id, contextWindow:__state.models[m.id].contextWindow, tpm:__state.models[m.id].tpm, rpm:__state.models[m.id].rpm, tpd:__state.models[m.id].tpd, enabled:__state.models[m.id].enabled })) }
