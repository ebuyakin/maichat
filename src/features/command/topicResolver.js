...
function splitPath(expr){
  ...
  if(!s) return []
  if(s.startsWith('/')){
    return s.split('/').filter(Boolean).map(x=>x.trim())
  }
...
export function resolveTopicFilter(value, { store, currentTopicId }){
  ...
  if(expr.endsWith('...')){ includeDesc = true; expr = expr.slice(0, -3) }
  expr = expr.trim()
  ...
  if(segments.length <= 1){
    ...
    if(!includeDesc){ return new Set(matched.map(t=>t.id)) }
  ...
  const matchingRoots = topics.filter(t=>{
    ...
    for(let i=0;i<reSegs.length;i++){ if(!reSegs[i].test(names[i] || '')) return false }
    return true
  })
  if(!includeDesc){ return new Set(matchingRoots.map(t=>t.id)) }
