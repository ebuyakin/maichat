...
export function lex(input){
  ...
  while(i < len){
    ...
    if(ch === '(' || ch === ')'){ push('PAREN', ch, start, ++i); continue }
    if(ch === '&' || ch === '|' || ch === '+' || ch === '!'){ push('OP', ch, start, ++i); continue }
    if(ch === '>' || ch === '<' || ch === '='){
      let val = ch; i++
      if((ch === '>' || ch === '<') && input[i] === '='){ val += '='; i++ }
      push('OP', val, start, i); continue
    }
    if(COMMANDS.has(ch)){
      // command letter; may be followed by number or quoted string parsed later by parser
      push('COMMAND', ch, start, ++i); continue
    }
    if(ch === '\'' || ch === '"'){
      const quote = ch; i++
      let str = ''
    ...
    if(/[a-zA-Z_]/.test(ch)){
      ...
      push('STRING', w, start, i); continue
    }
    // unknown char
    push('ERR', ch, start, ++i)
  }
