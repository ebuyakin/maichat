...
export function evaluate(ast, pairs, opts = {}) {
  ...
  function evalNode(node) {
    switch (node.type) {
      ...
      case 'AND':
        return intersect(evalNode(node.left), evalNode(node.right))
      case 'OR':
        return union(evalNode(node.left), evalNode(node.right))
      case 'NOT':
        return negate(evalNode(node.expr))
      default:
  ...
  function evalFilter(f) {
    ...
    switch (kind) {
      ...
      case 's':
        return filterStar(op, value)
      case 'r':
        return filterRecent(value)
      case 'b':
        return pairs.filter((p) => p.colorFlag === 'b')
      case 'g':
        return pairs.filter((p) => p.colorFlag === 'g')
      case 'e':
        return pairs.filter(
          (p) =>
      ...
      case 'd':
        return filterDate(op, value)
      case 'm':
        return filterModel(value)
      case 'c':
        return filterContent(value)
      default:
        return pairs
    }
  }
