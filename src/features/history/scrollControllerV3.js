...
export function createScrollController({ container, getParts }) {
  ...
  function scrollTo(target, animate) {
    ...
    if (!animate) {
      setScrollTopProgrammatic(target)
      return
    }
  ...
  function debugInfo() {
    ...
    return {
      paneH,
      currentFirst: vis.first,
      activeIndex: currentActiveIndex,
      shouldVisibleCount: vis.last - vis.first + 1,
      firstTopPx,
      visualGap,
      visibleIndices,
      tops,
      ...
      rawAnchor: anchorMeta.raw,
      maxScroll: anchorMeta.maxScroll,
      animationEnabled,
    }
  }
  ...
  function stepScroll(deltaPx) {
    if (!Number.isFinite(deltaPx)) return
    cancelAnimation()
    setScrollTopProgrammatic(container.scrollTop + deltaPx)
    scheduleValidate()
  }
  ...
  function indexByMessageId(messageId) {
    return findIndexById(messageId)
  }
  function alignToMessage(messageId, anchor = 'top', animate = false) {
    alignTo(messageId, anchor, animate)
  }
  function jumpToMessage(messageId, anchor = 'top', animate = false) {
    alignTo(messageId, anchor, animate)
  }
  function scrollToPosition(targetScroll, animate = false) {
    scrollTo(targetScroll, animate)
  }
  function expectScrollToBottom() {
    bootstrapScrollToBottomPending = true
  }
  return {
    remeasure: measure,
    apply,
