...
export function partitionMessage({ text, role, pairId }){
	...
	let paneH = 0
	try { if(typeof document !== 'undefined'){ const pane = document.getElementById('historyPane'); if(pane) paneH = pane.clientHeight } } catch {}
	const viewportH = paneH || win.innerHeight || 800
	const lineHeightPx = getPartLineHeight()
	let padTop = 0, padBottom = 0
	try { if(typeof document !== 'undefined'){ const pane = document.getElementById('historyPane'); if(pane){ const cs = window.getComputedStyle(pane); padTop = parseFloat(cs.paddingTop)||0; padBottom = parseFloat(cs.paddingBottom)||0 } } } catch {}
	...
	if(wrapWidth < 40) wrapWidth = Math.max(20, baseWidth * 0.5)
	const key = pairId+':'+role
	const textVersion = text.length
	...
	for(let lineIndex=0; lineIndex < rawLines.length;){
		...
		parts.push({ id: `${pairId}:${role}:${idx}`, pairId, role, index: idx, text: slice.join('\n'), lineStart: lineIndex, lineEnd: lineIndex + slice.length - 1, lineCount: slice.length, maxLinesUsed: maxLines })
...
function getPartLineHeight(){
	...
	try { if(typeof document === 'undefined') return 18; let probe = document.getElementById('mc-lineheight-probe'); if(!probe){ probe = document.createElement('div'); probe.id = 'mc-lineheight-probe'; probe.style.position='absolute'; probe.style.visibility='hidden'; probe.style.pointerEvents='none'; probe.style.zIndex='-1'; probe.style.whiteSpace='pre'; probe.className='part-inner'; probe.textContent='A'; document.body.appendChild(probe) } const lh = probe.getBoundingClientRect().height || 18; _lineHeightCache = lh; return lh } catch { return 18 }
}
function getHistoryContentWidth(){ if(typeof document === 'undefined') return 800; const pane = document.getElementById('historyPane'); if(!pane) return window.innerWidth * 0.8; const cs = window.getComputedStyle(pane); const padLeft = parseFloat(cs.paddingLeft)||0; const padRight = parseFloat(cs.paddingRight)||0; return pane.clientWidth - padLeft - padRight }
