...
export function runExport({
  ...
  const withPaths = rawPairs.map(p => ({
    id: p.id,
    createdAt: typeof p.createdAt === 'string' ? p.createdAt : new Date(p.createdAt||Date.now()).toISOString(),
    topicId: p.topicId || store.rootTopicId,
    topicPath: store.getTopicPath ? store.getTopicPath(p.topicId||store.rootTopicId).join(' > ') : '',
    model: p.model,
