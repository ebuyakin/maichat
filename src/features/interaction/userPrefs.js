...
export function saveCommandHistory(history){
  try{ localStorage.setItem(KEY_HISTORY, JSON.stringify((history||[]).slice(-MAX))) }catch{}
}
...
export function setFilterActive(v){ try{ localStorage.setItem(KEY_FILTER_ACTIVE, v ? 'true':'false') }catch{} }
export function getFilterActive(){ try{ return localStorage.getItem(KEY_FILTER_ACTIVE) === 'true' }catch{ return false } }
