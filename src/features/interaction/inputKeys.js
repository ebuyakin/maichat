...
export function createInputKeyHandler({
  ...
  return function inputHandler(e){
    ...
    if(e.ctrlKey && e.shiftKey && (e.key==='f' || e.key==='F')){ e.preventDefault(); const el = inputField; const pos = el.selectionStart; const text = el.value; const match = text.slice(pos).match(/\S+\s*/); if(match){ const newPos = pos + match[0].length; el.setSelectionRange(newPos, newPos); } return true }
    if(e.ctrlKey && e.shiftKey && (e.key==='b' || e.key==='B')){ e.preventDefault(); const el = inputField; const pos = el.selectionStart; const text = el.value.slice(0, pos); const match = text.match(/\s*\S+\s*$/); if(match){ const newPos = pos - match[0].length; el.setSelectionRange(newPos, newPos); } else { el.setSelectionRange(0, 0); } return true }
