...
export function createAppMenuController({
  ...
  function toggle(force){
    ...
    if(show){
      ...
      function onKey(e){
        ...
        if(e.key==='Enter'){
          const act=items[idx]||items[0]; if(act) activate(act)
          return
        }
      }
      try {
      ...
      function onHover(e){ const li = e.target && e.target.closest && e.target.closest('li'); if(!li || !m.contains(li)) return; const items = Array.from(m.querySelectorAll('li')); items.forEach(x=> x.classList.remove('active')); li.classList.add('active') }
      m.addEventListener('mouseover', onHover)
      backdrop.addEventListener('click', (e)=>{ if(m.contains(e.target)){ const li = e.target.closest('li[data-action]'); if(li){ e.stopPropagation(); activate(li) } return } close() })
    } else {
      close()
    }
  }
  ...
  function activate(li){ if(!li) return; const act = li.getAttribute('data-action'); close(); run(act) }
  ...
  function handleGlobalClick(){ const btn = menuBtn(); const m = menuEl(); if(!btn||!m) return; document.addEventListener('click', e=>{ if(e.target===btn){ e.stopPropagation(); toggle(); return } }) }
