...
export function createCommandKeyHandler({
  ...
  return function commandHandler(e){
    ...
    if(e.ctrlKey && e.shiftKey && (e.key==='f' || e.key==='F')){ e.preventDefault(); const el = commandInput; const pos = el.selectionStart; const text = el.value; const match = text.slice(pos).match(/\S+\s*/); if(match){ const newPos = pos + match[0].length; el.setSelectionRange(newPos, newPos); } return true }
    if(e.ctrlKey && e.shiftKey && (e.key==='b' || e.key==='B')){ e.preventDefault(); const el = commandInput; const pos = el.selectionStart; const text = el.value.slice(0, pos); const match = text.match(/\s*\S+\s*$/); if(match){ const newPos = pos - match[0].length; el.setSelectionRange(newPos, newPos); } else { el.setSelectionRange(0, 0); } return true }
    ...
    if(e.key==='Enter'){
      ...
      // HUD/debug toggles
      if(q === ':hud' || q === ':hud on'){ hudRuntime && hudRuntime.enable && hudRuntime.enable(true); commandInput.value=''; commandErrEl.textContent=''; return true }
      if(q === ':hud off'){ hudRuntime && hudRuntime.enable && hudRuntime.enable(false); commandInput.value=''; commandErrEl.textContent=''; return true }
      if(q === ':maskdebug' || q === ':maskdebug on'){ commandInput.value=''; commandErrEl.textContent=''; historyRuntime.applySpacingStyles(getSettings()); historyRuntime.updateFadeVisibility(); return true }
      if(q === ':maskdebug off'){ commandInput.value=''; commandErrEl.textContent=''; historyRuntime.applySpacingStyles(getSettings()); historyRuntime.updateFadeVisibility(); return true }
      if(q === ':anim off' || q === ':noanim' || q === ':noanim on'){ scrollController.setAnimationEnabled(false); commandInput.value=''; commandErrEl.textContent=''; return true }
      if(q === ':anim on' || q === ':noanim off'){ scrollController.setAnimationEnabled(true); commandInput.value=''; commandErrEl.textContent=''; return true }
      ...
      if(split && split.commandPart){
        ...
        try{
          ...
          const currentTopicPath = formatTopicPath(pendingMessageMeta.topicId || getCurrentTopicId())
          const environment = { currentModel: currentBareModel, currentTopicId: pendingMessageMeta.topicId || getCurrentTopicId(), currentTopicPath }
          const ui = {
          ...
          registry.run(cmd)
            .then(()=>{
              ...
              try {
                ...
                if(id && scrollController && scrollController.alignTo){
                  requestAnimationFrame(()=>{ requestAnimationFrame(()=>{ scrollController.alignTo(id, 'bottom', false) }) })
                }
              } catch {}
            ...
            .catch(ex=>{ const raw = (ex && ex.message) ? String(ex.message).trim() : 'error'; const friendly = `Command error: ${raw}`; commandErrEl.textContent = friendly })
        } catch(ex){ const raw = (ex && ex.message) ? String(ex.message).trim() : 'error'; const friendly = `Command error: ${raw}`; commandErrEl.textContent = friendly }
        return true
      }
      ...
      try {
        ...
        if(q){
          ...
          const currentBareModel = pendingMessageMeta.model || getActiveModel()
          const hasO = (node)=>{ if(!node) return false; if(node.type==='FILTER' && node.kind==='o') return true; if(node.type==='NOT') return hasO(node.expr); if(node.type==='AND'||node.type==='OR') return hasO(node.left)||hasO(node.right); return false }
          const stripO = (node)=>{ if(!node) return null; if(node.type==='FILTER' && node.kind==='o') return null; if(node.type==='NOT'){ const inner=stripO(node.expr); if(inner==null) return null; return { type:'NOT', expr: inner } } if(node.type==='AND'||node.type==='OR'){ const l=stripO(node.left); const r=stripO(node.right); if(!l&&!r) return null; if(!l) return r; if(!r) return l; return { type: node.type, left:l, right:r } } return node }
          if(hasO(ast)){
            const baseAst = stripO(ast) || { type:'ALL' }
        ...
        if(!activeParts.parts.length){
          // no parts shown
          pushCommandHistory(q); modeManager.set('view'); return true
        }
