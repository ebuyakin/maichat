...
export function createTopicPicker({ store, modeManager, onSelect, onCancel }){
  ...
  function toggleExpandOrDescend(){
    ...
    buildFlat(); const firstChildId = [...kids][0]
    const idx = flat.findIndex(r=>r.topic.id===firstChildId); if(idx>=0){ activeIndex = idx; render() }
  }
  function collapseOrParent(){
    ...
    buildFlat(); const pIdx = flat.findIndex(r=>r.topic.id===parentId); if(pIdx>=0){ activeIndex = pIdx; render() }
  }
  ...
  searchInput.focus(); render(); return { destroy: teardown }
}
