...
export function createChronoTopicPicker({ 
  ...
  rootEl.style.cssText = 'position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.7);z-index:500;display:flex;align-items:center;justify-content:center;'
  
  const panel = document.createElement('div')
  panel.className = 'chrono-topic-picker'
  panel.style.cssText = 'background:var(--bg-alt);border:1px solid #333;border-radius:8px;padding:20px;min-width:400px;max-width:600px;max-height:60vh;display:flex;flex-direction:column;'
  ...
  topicIds.forEach((topicId, index) => {
    ...
    item.style.cssText = 'padding:8px 12px;border-radius:4px;cursor:pointer;display:flex;justify-content:space-between;align-items:center;'
