...
export function openTopicEditor({ store, onSelect, onClose }) {
  ...
  function buildFlat() {
    ...
    const match = (t) =>
      ...
      topicPathNames(t.id).toLowerCase().includes(lowerFilter)
  ...
  function currentTopic() {
    return flat[activeIndex]?.topic || null
  }
  ...
  function saveDetails() {
    ...
    if (!Number.isNaN(tempVal)) rp.temperature = Math.max(0, Math.min(2, tempVal))
    else delete rp.temperature
  ...
  function insertPath() {
    const t = currentTopic()
    if (!t) return
    const ins = topicPathNames(t.id)
    const el = sysTextarea
  ...
  function ensureVisible(topicId) {
    ...
    while (cur && cur.parentId) {
      ...
      cur = store.topics.get(cur.parentId)
    }
  }
  function createChild() {
    ...
    render()
    const input = treeEl.querySelector('input.te-edit')
    if (input) {
      editing.inputEl = input
      input.focus()
    }
  }
  ...
  function finishCreate(commit) {
    ...
    if (commit) {
      ...
      if (siblingNameExists(parentId, val)) {
        showWarning('Duplicate sibling name')
        return
      }
      ...
      filter = ''
      buildFlat()
      activeIndex = flat.findIndex((r) => r.topic.id === id)
      render()
    } else {
      editing = null
      render()
  ...
  function finishRename(commit) {
    ...
    if (commit) {
      ...
      if (siblingNameExists(t.parentId, newName, t.id)) {
        showWarning('Duplicate sibling name')
        return
      }
      if (newName !== t.name) {
        store.renameTopic(t.id, newName)
        sessionDirty = true
      }
    }
    editing = null
  ...
  function attemptDelete() {
    const t = currentTopic()
    if (!t) return
    if (t.id === store.rootTopicId) {
      showWarning('Cannot delete root')
      return
    }
  ...
  function executeDelete(id) {
    ...
    if (!ok) {
      showWarning('Delete blocked (children or messages)')
      return
    }
  ...
  function placeTopic() {
    if (!markedTopicId) {
      showWarning('Nothing marked')
      return
    }
    const movingId = markedTopicId
    const target = currentTopic()
    if (!target) {
      showWarning('No target')
      return
    }
    if (target.id === movingId) {
      showWarning('Cannot paste into itself')
      return
    }
    const ok = store.moveTopic(movingId, target.id)
    if (!ok) {
      showWarning('Move blocked (cycle/root)')
      return
    }
    ...
    sessionDirty = true
    buildFlat()
    activeIndex = flat.findIndex((r) => r.topic.id === movingId)
  ...
  function onKey(e) {
    if (editing) {
      ...
      if (e.key === 'Enter') {
        e.preventDefault()
        if (editing.mode === 'create') finishCreate(true)
        else finishRename(true)
        setTimeout(() => {
          inTreeFocus ? treeEl.focus() : searchInput.focus()
        }, 0)
        return
      }
      return
  ...
  function moveActiveSibling(delta) {
    ...
    buildFlat()
    const newIdx = flat.findIndex((r) => r.topic.id === t.id)
  ...
  backdrop.addEventListener(
    ...
    (e) => {
      ...
      if (e.ctrlKey && (e.key === 'e' || e.key === 'E')) {
        ...
        e.stopPropagation()
        e.stopImmediatePropagation()
        detailsEl.querySelector('.te-textarea')?.focus()
        inTreeFocus = false
        return
      }
  ...
  backdrop.addEventListener(
    ...
    (e) => {
      ...
      if (orderMode === 'manual' && (k === 'u' || k === 'U')) {
        ...
        e.stopImmediatePropagation()
        moveActiveSibling(-1)
      } else if (orderMode === 'manual' && (k === 'd' || k === 'D')) {
        e.preventDefault()
        e.stopPropagation()
        e.stopImmediatePropagation()
        moveActiveSibling(1)
  ...
  backdrop.addEventListener(
    'focusin',
    (e) => {
      inTreeFocus = treeEl.contains(e.target)
    },
    true
  )
