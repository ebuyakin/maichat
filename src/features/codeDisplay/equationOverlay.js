...
export function createEquationOverlay({ modeManager }) {
  ...
  function renderPanel(eqBlock){
    ...
    const idxDisplay = total>1 ? `<span class="code-overlay-index">(${currentIndex+1}/${total})</span>` : '';
    ...
    if(copyBtn){ copyBtn.addEventListener('click', ()=> copyToClipboard(eqBlock.tex)) }
  }
  ...
  function show(eqBlock, messagePair, options={}){
    ...
    if(messagePair && Array.isArray(messagePair.equationBlocks)){
      ...
      if(options.index!=null){ currentIndex = options.index }
      else {
    ...
    if(modalHandle){
      renderPanel(currentBlock); focus(); return;
    }
    ...
    panel.addEventListener('keydown', (e)=>{
      if((e.ctrlKey||e.metaKey) && e.key==='c'){
        ...
        if(blocks && blocks.length>1){
          e.preventDefault();
          if(e.key==='n'){ currentIndex = (currentIndex+1) % blocks.length }
          else { currentIndex = (currentIndex-1+blocks.length) % blocks.length }
          currentBlock = blocks[currentIndex];
          renderPanel(currentBlock);
          focus();
        }
      } else if(!e.ctrlKey && !e.metaKey && !e.altKey && (e.key==='j' || e.key==='k' || e.key==='d' || e.key==='u' || e.key==='ArrowDown' || e.key==='ArrowUp')){
    ...
    focus();
    console.log('[EquationOverlay] Opened modal for block:', currentBlock.display ? 'display' : 'inline');
  }

  function focus(){ try { const el = panel && panel.querySelector('.code-overlay-pre'); if(el) el.focus() } catch{}
  }
  function close(trigger){ if(!modalHandle){ return } try { modalHandle.close(trigger) } catch {}; console.log('[EquationOverlay] Closed'); }
  function isVisible(){ return !!modalHandle }
  function copyToClipboard(text){
    if(!text) return;
    if(navigator.clipboard){
      navigator.clipboard.writeText(text).catch(err=> console.error('[EquationOverlay] Copy failed', err));
    } else {
