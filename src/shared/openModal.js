...
function isWithin(el, root){
  try { return !!(root && el && root.contains(el)) } catch { return false }
}
function isInAnyExempt(el){
  for(const er of exemptRoots){ if(isWithin(el, er)) return true }
  return false
}
...
function shouldBlockByType(e, policy){
  ...
  if(t === 'mousedown' || t === 'mouseup' || t === 'click' || t === 'contextmenu' || t === 'pointerdown' || t === 'pointerup') return !!policy.pointer
...
export function registerModalExemptRoot(root){
  ...
  return ()=>{ try{ exemptRoots.delete(root) }catch{} }
}

export function openModal({ modeManager, root, preferredFocus, closeKeys=['Escape'], restoreMode=true, onCloseKey, beforeClose, modal=true, blockPolicy }){
  ...
  if(stackEntry){ modalStack.push(stackEntry); installBlockers() }
...
try {
  if(typeof window !== 'undefined'){
    window.modalIsActive = function(){ return modalStack.length > 0 }
  }
} catch {}
