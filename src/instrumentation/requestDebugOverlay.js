...
export function createRequestDebugOverlay({ historyRuntime }) {
  ...
  function ensureOverlay() {
    ...
    try {
      if (!pane.__exempt) {
        pane.__exempt = registerModalExemptRoot(pane)
      }
    } catch {}
  }
  ...
  function render() {
    ...
    if (!lastSentRequest) {
      pane.textContent = '[request debug] No request sent yet.'
      return
    }
    const {
      model,
    ...
    const uraVal =
      (ctxStats && ('URA' in ctxStats ? ctxStats.URA : ctxStats.assumedUserTokens)) ??
      settings.userRequestAllowance
    const cpt = settings.charsPerToken
    const nta = settings.maxTrimAttempts
    const ml = budget.maxContext || budget.maxUsableRaw
    const initialAttemptTotal =
      predictedHistoryTokens != null && AUT != null ? predictedHistoryTokens + AUT : null
    const finalAttemptTotal =
      attemptHistoryTokens != null && AUT != null ? attemptHistoryTokens + AUT : null
    const trimmedTok =
      predictedHistoryTokens != null && attemptHistoryTokens != null
        ? predictedHistoryTokens - attemptHistoryTokens
        : 0
    const lines = []
    ...
    if (symbols) {
      ...
      lines.push(
        `  HLP=${HLP} HLA=${HLA} H0=${H0 != null ? H0 : '-'} H=${H != null ? H : '-'} R=${remainingContext != null ? remainingContext : '-'}`
      )
    ...
    try {
      ...
      if (
        ...
        window.__maichatLastRequest.json
      ) {
        try {
          parsed = JSON.parse(window.__maichatLastRequest.json)
        } catch {}
      }
      if (parsed && typeof parsed === 'object') {
        ...
        if (temp != null || maxTok != null) {
          lines.push(
            `OPENAI OPTIONS:${temp != null ? ` temperature=${temp}` : ''}${maxTok != null ? ` max_tokens=${maxTok}` : ''}`
          )
        }
      }
    } catch {}
    lines.push(
      `PREDICTED_HISTORY_CONTEXT: n_of_messages=${predictedMessageCount} n_of_tokens=${predictedHistoryTokens != null ? predictedHistoryTokens : '-'}`
    )
    lines.push(`ACTUAL:`)
    lines.push(`  tokens_in_new_user_request=${AUT != null ? AUT : '-'}`)
    lines.push(
      `  tokens_in_initial_attempted_request=${initialAttemptTotal != null ? initialAttemptTotal : '-'}`
    )
    lines.push(`TRIMMING:`)
    lines.push(`  N_of_attempts=${attemptsUsed != null ? attemptsUsed : 0}`)
    lines.push(`  N_of_tokens_trimmed=${trimmedTok}`)
    lines.push(
      `  tokens_in_final_attempted_request=${finalAttemptTotal != null ? finalAttemptTotal : '-'}`
    )
    lines.push(`  remaining_estimate=${remainingReserve != null ? remainingReserve : '-'}`)
    if (timing) {
      const tPredict =
        timing.tAfterPrediction != null ? (timing.tAfterPrediction - timing.t0).toFixed(1) : '-'
      lines.push(`TIMING:`)
      ...
      if (Array.isArray(timing.attempts)) {
        timing.attempts.forEach((a) => {
          ...
          lines.push(
            `  attempt_${a.attempt}_ms=${dur} trimmedCountAtStart=${a.trimmedCount}${prov}`
          )
        })
      }
    }
    if (lastErrorMessage) {
      lines.push(
        `ERROR: msg="${lastErrorMessage}" overflowMatched=${overflowMatched ? '1' : '0'} stage=${stage || '-'}`
      )
    }
    ...
    if (!pane.__hasCopy) {
      ...
      btn.style.top = '4px'
      btn.style.right = '6px'
      btn.style.font = '10px var(--font-ui)'
      btn.style.padding = '2px 6px'
      btn.style.background = '#123a55'
      btn.style.border = '1px solid #25506f'
      btn.style.color = '#cce'
    ...
    if (!pre) {
      ...
      pre.style.margin = '0'
      pre.style.padding = '0 0 4px'
      pre.style.font = '11px/1.4 var(--font-mono, monospace)'
  ...
  function toggle() {
    requestDebugEnabled = !requestDebugEnabled
    render()
  }
  function enable(v) {
    requestDebugEnabled = !!v
    render()
  }
