...
export function createHudRuntime({
  store,
  activeParts,
  scrollController,
  historyPaneEl,
  ...
  try {
    if (!hudEl.__exempt) {
      hudEl.__exempt = registerModalExemptRoot(hudEl)
    }
  } catch {}
  ...
  if (!hudEl.__hudClickBound) {
    hudEl.addEventListener('click', (e) => {
      ...
      if (key && hudState.hasOwnProperty(key)) {
  ...
  function update() {
    ...
    const focusDesc = focusEl
      ? `${focusEl.tagName.toLowerCase()}${focusEl.id ? '#' + focusEl.id : ''}`
      : 'none'
    ...
    layoutParams.push(
      `${n++}. Outer gap (paddingTop): ${historyEl ? parseFloat(getComputedStyle(historyEl).paddingTop) || 0 : '?'}`
    )
