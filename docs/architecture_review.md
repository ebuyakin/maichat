...
| History Rendering Runtime | Build parts, render history, layout pane sizing, active part update, fading, context stats integration, status indicators | `features/history/historyRuntime.js`, `features/history/historyView.js`, `features/history/parts.js` |
| Interaction Layer | Mode-aware key routing, command filter application, command history, star/flag/model/topic actions, send triggers, menu & overlays dispatch | `ui/interaction/interaction.js` (pre-move), `ui/modes.js`, `ui/keyRouter.js` |
...
| Partitioning & Scroll Mechanics | Logical line wrapping into parts, active part controller, anchor-based scroll invariants | `features/history/partitioner.js`, `features/history/scrollControllerV3.js`, `features/history/parts.js` |
| Overlays (Modal UI) | Topical editors, pickers, settings, model management, API key input, help | `ui/topicEditor.js`, `ui/topicPicker.js`, `ui/settingsOverlay.js`, `ui/modelSelector.js`, `ui/modelEditor.js`, `ui/apiKeysOverlay.js`, `ui/helpOverlay.js`, `shared/openModal.js`, `shared/focusTrap.js` |
...
Design Rationale:
* Function factories over classes for ergonomic dependency injection.
* Deferred event bus until multiple observer need emerges.
* Diagnostics isolated for easy production build stripping.
