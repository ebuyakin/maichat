...
P6. Historical images in payload. Enable AI to reference images from conversation history by embedding image references in correct message objects instead of flat array. [x]
P6.1 Revert sendWorkflow.js historical image collection logic from first attempt. Files: features/compose/sendWorkflow.js [x]
P6.2 Modify buildMessages() to extract and embed image IDs from pair data into each message object. Files: features/compose/pipeline.js [x]
P6.3 Update executeSend() to pass new message attachments to buildMessages() and remove global attachments from provider call. Files: features/compose/pipeline.js [x]
P6.4 Add telemetry to track image distribution across messages in payload for debugging. Files: features/compose/pipeline.js [x]
P6.5 Update Gemini adapter to read attachments from message objects instead of global array. Files: infrastructure/provider/ge iniAdapter.js [x]
P6.6 Test end-to-end with Gemini - verify images appear in correct historical messages using telemetry. [x]
P6.7 Update OpenAI adapter to match Gemini pattern. Files: infrastructure/provider/openaiAdapter.js [x]
P6.8 Update Anthropic adapter to match Gemini pattern. Files: infrastructure/provider/anthropicAdapter.js [x]
P6.9 Update Grok adapter to match Gemini pattern. Files: infrastructure/provider/grokAdapter.js [x]
